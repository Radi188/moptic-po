...
api.interceptors.request.use(async (config) => {
  ...
  if (__DEV__) {
    ...
    (config as { metadata?: { start: number } }).metadata = {
      start: Date.now(),
    };
  }
  return config;
...
api.interceptors.response.use(
  ...
  async (
    error: AxiosError<{ message?: string; error?: unknown; errors?: unknown }>,
  ) => {
    if (__DEV__) {
    ...
    return Promise.reject(
      new ApiError(extractMessage(error), error.response?.status ?? 0),
    );
  },
);
...
function logTiming(
  ...
  const start = (config as { metadata?: { start: number } } | undefined)
    ?.metadata?.start;
...
function extractMessage(
  ...
  if (data) {
    // Validation errors ({ errors: { field: [msg] } }). Prefer the specific
    // field message on 422 over the generic top-level message.
    const bag = data.errors ?? data.error;
    if (
      (status === 422 || typeof data.message !== "string") &&
      bag &&
      typeof bag === "object"
    ) {
      const first = Object.values(bag as Record<string, unknown>)[0];
