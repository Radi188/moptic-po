...
import { fetchTransfersIn, type TransferInCategory, type TransferInItem } from '@/api/transfers-in';
...
export default function TransferInReportScreen() {
  ...
  const [categories, setCategories] = useState<TransferInCategory[]>([]);
  ...
  const load = useCallback(
    async (from: Date, to: Date, branchId: string | undefined, q: string) => {
      ...
      setError(null);
      try {
        const report = await fetchTransfersIn({
          dateFrom: ymd(from),
          dateTo: ymd(to),
          branchId,
          search: q,
        ...
        setItemCount(report.itemCount);
        setCategoryCount(report.categoryCount);
      ...
      <Pressable onPress={onToggle} style={({ pressed }) => pressed && styles.pressed}>
        <ThemedView type="backgroundElement" style={styles.categoryHeader}>
          ...
          <View style={styles.qtyWrap}>
            <ThemedText type="smallBold" style={{ color: '#30A46C', fontSize: 18 }}>
